...
下载课程资料的xpath 压缩包`2.0.20.zip`

打开谷歌浏览器

![在这里插入图片描述](https://img-blog.csdnimg.cn/20200505000824658.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80NDY3MzA0Mw==,size_16,color_FFFFFF,t_70)

点击扩展程序后界面如下

![在这里插入图片描述](https://img-blog.csdnimg.cn/20200505001131526.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80NDY3MzA0Mw==,size_16,color_FFFFFF,t_70)

把压缩包拖拽过来即可安装

如果不成功可以解压然后选择加载已经解压好的文件

成功安装后重启浏览器，会出现如下图标

![在这里插入图片描述](https://img-blog.csdnimg.cn/20200505001527762.png)
...
在这里我们是逐层选取的，先选取了 li 节点，又利用 / 选取了其直接子节点 a，然后再选取其文本，得到的结果恰好是符合我们预期的五个结果。
...
不出所料，这里返回结果是六个，可想而知这里是选取所有子孙节点的文本，其中前两个就是 li 的子节点 a 节点内部的文本，另外一个就是最后一个 li 节点内部的文本，即换行符。
...
```
# 编剧
//span[text()="编剧"]/following-sibling::span[1]/a
# 导演
//span[text()="导演"]/following-sibling::span[1]/a
# 主演
//span[@class="actor"]//span[@class="attrs"]
# 多值匹配语法
//span[contains(text(),'编')]/following-sibling::span[1]/a
```
