...
先通过下面一行命令安装好`fake_useragent`包：

```
pip3 install fake-useragent==0.1.10
```

修改useragent 文件权限

```
sudo cp /home/learn/code/fake_useragent_0.1.11.json /tmp/fake_useragent_0.1.11.json
sudo chmod 777 /tmp/fake_useragent_0.1.11.json
```

使用 ua.random 属性，可以随机生成各种浏览器的 UA：
...
- 代理失效会报错requests.exceptions.ProxyError: HTTPConnectionPool
...
data = {
    ...
    'login': '13166297976@163.com',
    'password': 'eHVrYWkxMjMxMjM='
}
response = session.post(post_url, headers=headers, data=data)
with open('git_post.html', 'wb') as F:
